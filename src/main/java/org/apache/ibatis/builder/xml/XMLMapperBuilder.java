...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  private ResultMap resultMapElement(XNode resultMapNode, List<ResultMapping> additionalResultMappings, Class<?> enclosingType) throws Exception {
    ...
    // <1> 获得 type 属性  获得 id、type、extends、autoMapping 属性，并解析 type 对应的类型。
    ...
    // <1> 获得 id 属性 获得 id、type、extends、autoMapping 属性，并解析 type 对应的类型。
    String id = resultMapNode.getStringAttribute("id",
            resultMapNode.getValueBasedIdentifier());
    // <1> 获得 extends 属性 获得 id、type、extends、autoMapping 属性，并解析 type 对应的类型。
    String extend = resultMapNode.getStringAttribute("extends");
    // <1> 获得 autoMapping 属性 获得 id、type、extends、autoMapping 属性，并解析 type 对应的类型。
  ...
  private void processConstructorElement(XNode resultChild, Class<?> resultType, List<ResultMapping> resultMappings) throws Exception {
    ...
    for (XNode argChild : argChildren) {
      ...
      // <3> 将当前子节点构建成 ResultMapping 对象，并添加到 resultMappings
  ...
  private void bindMapperForNamespace() {
    ...
    if (namespace != null) {
      ...
      if (boundType != null) {
        // <2> 不存在该 Mapper 接口，则进行添加  调用 Configuration#hasMapper(Class<?> type) 方法，判断若谷不存在该 Mapper 接口，则进行绑定
